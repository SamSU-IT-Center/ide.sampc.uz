var defaultUrl = localStorageGetItem("api-url") || "http://api.sampc.uz/";
var extraApiUrl = "http://api.sampc.uz/";
...
function handleResult(data) {
    ...
    const compileTime = data.compileTime == null ? '-': data.compileTime + "s";
    $statusLine.html(`${status}, ${time}, ${memory} (TAT: ${tat}ms), Compiler Time: ${compileTime}`);
...
function run() {
    ...
    var languageX = $("#select-language option:selected").text().trim().split(' ')

    var version = languageX[1].replace(/[()]/g, '');
    var language = languageX[0];
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data) {
                    ...
                    setTimeout(fetchSubmission.bind(null, data, 1), INITIAL_WAIT_TIME_MS);
                }
            },
    ...
    var settings = {
      ...
      $.ajax(settings).done(function (response) {
        for (let index = 0; index < response.length; index++) {
            ...
            if (index == 0) {
                options += `
                    <option value="${map[element.language][element.version].value}" mode="${map[element.language][element.version].mode}" selected>
                        ${element.language} (${element.version})
                    </option>
                `
            } else {
                options += `
                    <option value="${map[element.language][element.version].value}" mode="${map[element.language][element.version].mode}">
                        ${element.language} (${element.version})
                    </option>
                `
            }
...
IT Center IDE assumes that content of executable is Base64 encoded.\n\
\n\
...
var sqliteSource = "\
-- On IT Center IDE your SQL script is run on chinook database (https://www.sqlitetutorial.net/sqlite-sample-database).\n\
-- For more information about how to use SQL with IT Center please\n\
...
var c3Source = "\
// On the IT Center IDE, C3 is automatically\n\
...
var nimSource = "\
# On the IT Center IDE, Nim is automatically\n\
...
var bosqueSource = "\
// On the IT Center IDE, Bosque (https://github.com/microsoft/BosqueLanguage)\n\
